...
async function main() {
  ...
  try {
    ...
    const ADA = "dev:player-ada-0001",
      BORA = "dev:player-bora-002";
    ...
    ada.socket.emit(EV.READY, true);
    bora.socket.emit(EV.READY, true);
    ...
    log(
      betState.bet !== null && betState.bet.category.length > 0,
      `bahis fazında kategori görünür (${betState.bet?.category})`,
    );
    ...
    const auto = (c: Client) =>
      c.socket.on(EV.STATE, (s: GameState) => {
