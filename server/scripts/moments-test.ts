...
const internals = (room: Room) => room as unknown as {
  ...
  revealUntil: number
  phase: string
  players: Map<string, { stats: { bestStreak: number; fastestMs: number | null; maxGain: number; correct: number; total: number }; isBot: boolean }>
}

/** Maçı oynayıp podyuma düşürür; herkes doğru cevaplar. */
const finishRoom = (id: string, ids: string[], opts: { answerCorrect?: (id: string, qIndex: number) => boolean } = {}) => {
  ...
  while (inner.phase !== 'podium') {
    ...
    inner.reveal()
    inner.revealUntil = 0
    if (inner.qIndex >= total - 1) { inner.qIndex = total - 1; inner.advanceFromReveal(); break }
    inner.qIndex += 1
...
test('Bot anlara dahil edilmez', () => {
  ...
  assert.ok(moments.every((m) => m.playerId !== 'bot'), 'bot anı olmamalı')
})
