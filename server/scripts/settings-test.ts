...
test("Host değilse süre değiştiremez", () => {
  ...
  assert.throws(
    () => r.setQuestionTime("b", 10_000),
    (e: unknown) => e instanceof GameError && e.key === "err.timeHostOnly",
  );
});
...
test("Geçersiz süre reddedilir", () => {
  ...
  assert.throws(
    () => r.setQuestionTime("a", 7_000),
    (e: unknown) => e instanceof GameError && e.key === "err.timeInvalid",
  );
});
...
test("Süre ayarı soruya uygulanır", () => {
  ...
  assert.ok(
    inner.questionDeadline - Date.now() <= 10_500,
    `deadline ${inner.questionDeadline - Date.now()} 10sn üstünde`,
  );
...
test("Non-boolean bayrak reddedilir", () => {
  ...
  assert.throws(
    () => r.setTableFlag("a", "imageOnly", "yes"),
    (e: unknown) => e instanceof GameError && e.key === "err.settingInvalid",
  );
});
...
test("Sayısız modlarda soru sayısı reddedilir (duel/word/blitz/board)", () => {
  for (const mode of ["duel", "word", "blitz", "board"] as const) {
    ...
    assert.throws(
      () => r.setQuestionCount("a", 10),
      (e: unknown) => e instanceof GameError && e.key === "err.countMode",
    );
  }
});
...
test("Sayı destekli modda çalışmaya devam eder", () => {
  ...
  assert.throws(
    () => c.setQuestionCount("a", 5),
    (e: unknown) => e instanceof GameError && e.key === "err.countInvalid",
  );
});
