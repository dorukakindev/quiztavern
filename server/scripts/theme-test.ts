...
test("Host değilse tema değiştiremez", () => {
  ...
  assert.throws(
    () => r.setTableTheme("b", "forest"),
    (e: unknown) => e instanceof GameError && e.key === "err.themeHostOnly",
  );
});
...
test("Geçersiz tema reddedilir", () => {
  ...
  assert.throws(
    () => r.setTableTheme("a", "neon"),
    (e: unknown) => e instanceof GameError && e.key === "err.themeInvalid",
  );
});
...
test("Ligi yetmeyen tema kilitli", () => {
  ...
  assert.throws(
    ...
    (e: unknown) => e instanceof GameError && e.key === "err.themeLocked",
  );
  store!.close();
});
