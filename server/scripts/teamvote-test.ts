...
const teamRoom = (ids: string[]) => {
  ...
  names.forEach((id, i) => { inner.players.get(id)!.team = i < Math.ceil(ids.length / 2) ? 0 : 1 })
...
test('Eşitlikte kaptan (düşük seat) seçimi takım cevabı olur', () => {
  ...
  r.answer('a', cq.correctIndex); r.answer('b', wrong)
  // takım1 (c,d): ikisi de yanlış
  r.answer('c', wrong); r.answer('d', wrong)
  inner2.reveal()
...
test('Eşitlikte kaptan yanılırsa takım puanı yok', () => {
  ...
  const cq = r.currentQuestion()!
  const wrong = (cq.correctIndex + 1) % 4
  r.answer('a', wrong); r.answer('b', cq.correctIndex)
  r.answer('c', cq.correctIndex); r.answer('d', cq.correctIndex)
  inner.reveal()
