...
test("Klasik maçta yazılan soru havuza karışır (written-<id>)", () => {
  ...
  assert.ok(
    inner.questions.some((q) => q.id === "written-a"),
    "written-a havuzda yok",
  );
...
test("Dar havuzda yazar slotu dizi sınırını aşmaz — delik yok (B59)", () => {
  ...
  for (const pid of ["a", "b", "c", "d"]) {
    r.setReady(pid, true);
    r.submitQuestion(pid, good({ text: `yazar ${pid} sorusu uzun metin` }));
  }
  r.start("a", "classic");
  const inner = internals(r);
