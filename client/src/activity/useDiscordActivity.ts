...
function closeSdk(sdk: DiscordSDK, message: string) {
  try {
    sdk.close(RPCCloseCodes.CLOSE_ABNORMAL, message);
  } catch {
    /* zaten kapanmış */
  }
}
  ...
  useEffect(() => {
    ...
    connectOnce(clientId)
      ...
      .catch((nextError: unknown) => {
        ...
        console.error(
          `[activity] ${step} adiminda dustu:`,
          nextError instanceof AuthStepError ? nextError.reason : nextError,
        );
      });
