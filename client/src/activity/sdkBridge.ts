...
export function subscribeSpeaking(sdk: DiscordSDK, apply: (speakingIds: Set<string>) => void): () => void {
  ...
  const onStart = ({ user_id }: { user_id: string }) => {
    speaking.add(user_id);
    emit();
  };
  const onStop = ({ user_id }: { user_id: string }) => {
    speaking.delete(user_id);
    emit();
  };
...
export async function inviteWithFallback(sdk: DiscordSDK, message: string): Promise<boolean> {
  if (sdk.guildId) {
    try {
      ...
      /* izin yok — link paylaşımına düş */
    }
  }
  try {
    return (await sdk.commands.shareLink({ message })).success;
  } catch {
    return false;
  }
}
...
export async function shareResult(sdk: DiscordSDK, message: string): Promise<boolean> {
  try {
    ...
    /* link paylaşımı yok — davet diyaloğuna düş */
  }
  if (sdk.guildId) {
    try {
      await sdk.commands.openInviteDialog();
