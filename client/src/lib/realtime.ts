...
import { EV, type BadgeKey, type CardType, type Difficulty, type EmoteKey, type EmotePayload, type GameMode, type GameState, type TableTheme, type ToastPayload, type WrittenQuestionInput } from '../../../shared/types'
...
export function useRealtimeGame(roomId = 'ana-lobi', identity?: ActivityRealtimeIdentity, onAuthRequired?: () => void) {
  ...
  return {
    ...
    sendEmote: (emote: EmoteKey) => { if (socket.connected) socket.emit(EV.EMOTE, { emote }) },
    reconnectNow: () => { if (!socket.connected) { setStatus('connecting'); setConnectionError(null); socket.connect() } },
    start: (mode: GameMode = 'quiz') => { if (socket.connected) socket.emit(EV.START, { mode }) },
    startDaily: () => { if (socket.connected) socket.emit(EV.START, { daily: true }) },
    ...
    answerOrder: (order: number[]) => { if (socket.connected) socket.emit(EV.ORDER_ANSWER, order) },
    pickCell: (cell: number) => { if (socket.connected) socket.emit(EV.PICK_CELL, cell) },
    ...
    playAgain: () => { if (socket.connected) socket.emit(EV.PLAY_AGAIN) },
    addBot: () => { if (socket.connected) socket.emit(EV.ADD_BOT) },
    ready: (value: boolean) => { if (socket.connected) socket.emit(EV.READY, value) },
    setCategories: (categories: string[]) => { if (socket.connected) socket.emit(EV.SET_CATEGORIES, categories) },
    setQuestionCount: (count: number) => { if (socket.connected) socket.emit(EV.SET_QUESTION_COUNT, { count }) },
    setDifficulty: (difficulty: Difficulty | null) => { if (socket.connected) socket.emit(EV.SET_DIFFICULTY, { difficulty }) },
    setPack: (packId: string | null) => { if (socket.connected) socket.emit(EV.SET_PACK, { packId }) },
    setTableTheme: (theme: TableTheme) => { if (socket.connected) socket.emit(EV.SET_THEME, { theme }) },
    submitQuestion: (q: WrittenQuestionInput) => { if (socket.connected) socket.emit(EV.SUBMIT_QUESTION, q) },
    deleteQuestion: () => { if (socket.connected) socket.emit(EV.DELETE_QUESTION) },
    setQuestionTime: (ms: number | null) => { if (socket.connected) socket.emit(EV.SET_QUESTION_TIME, { ms }) },
    setSpeedBonus: (value: boolean) => { if (socket.connected) socket.emit(EV.SET_SPEED_BONUS, { value }) },
    setImageOnly: (value: boolean) => { if (socket.connected) socket.emit(EV.SET_IMAGE_ONLY, { value }) },
    setMode: (mode: GameMode) => { if (socket.connected) socket.emit(EV.SET_MODE, { mode }) },
    setTeam: (targetId: string, team: number) => { if (socket.connected) socket.emit(EV.SET_TEAM, { targetId, team }) },
    shuffleTeams: () => { if (socket.connected) socket.emit(EV.TEAM_SHUFFLE) },
    ...
    kick: (targetId: string) => { if (socket.connected) socket.emit(EV.KICK, { targetId }) },
    transferHost: (targetId: string) => { if (socket.connected) socket.emit(EV.TRANSFER_HOST, { targetId }) },
    // İzleyici modu: koltuğu bırak (izle) / boş koltuğa otur (oyna). Karar sunucuda.
    spectate: () => { if (socket.connected) socket.emit(EV.SPECTATE) },
    takeSeat: () => { if (socket.connected) socket.emit(EV.TAKE_SEAT) },
    ...
    predict: (targetId: string) => { if (socket.connected) socket.emit(EV.PREDICT, { targetId }) },
    // Unvan tak/kaldır (null = kaldır) — kazanılmış rozetlerden biri olmalı.
    setTitle: (title: BadgeKey | null) => { if (socket.connected) socket.emit(EV.SET_TITLE, { title }) },
    /**
    ...
    leaveGame: (thenRejoin = false) => {
      ...
      if (thenRejoin) {
        ...
        window.setTimeout(() => { if (!rejoined && socket.connected) socket.disconnect() }, 2000)
      }
    ...
    returnToLobby: () => { if (socket.connected) socket.emit(EV.RETURN_TO_LOBBY) },
    /** Podyumda rövanş oyu — çoğunluk sağlanırsa sunucu yeni maçı başlatır. */
    rematch: () => { if (socket.connected) socket.emit(EV.REMATCH) },
