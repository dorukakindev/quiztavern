...
const SOUNDS: Record<SfxName, (c: AudioContext) => void> = {
  ...
  lock: (c) => {
    note(c, 620, 0, 0.06, "triangle", 0.16);
    note(c, 900, 0.045, 0.07, "triangle", 0.14);
  },
  // Süre tiki: kısa, alçak — son saniyelerde gerilim.
  tick: (c) => {
    note(c, 1050, 0, 0.035, "triangle", 0.1);
  },
  // Doğru: yükselen üçlü arpej (do-mi-sol) — ödül.
  correct: (c) => {
    [523, 659, 784].forEach((f, i) => note(c, f, i * 0.06, 0.15, "triangle", 0.15));
  },
  // Yanlış: alçalan mat ton.
  wrong: (c) => {
    note(c, 300, 0, 0.16, "sawtooth", 0.1);
    note(c, 160, 0.05, 0.22, "sine", 0.11);
  },
  // Reveal vuruşu: kısa alçak tok ses.
  reveal: (c) => {
    note(c, 150, 0, 0.28, "sine", 0.18);
  },
  // Podyum fanfarı: yükselen dörtlü.
  podium: (c) => {
    [523, 659, 784, 1046].forEach((f, i) => note(c, f, i * 0.085, 0.32, "triangle", 0.14));
  },
  // Son 3 saniye kalp atışı: tick'in üstüne katmanlanır, 3->2->1 giderek ağırlaşır/güçlenir.
  heart: (c) => {
    note(c, 90, 0, 0.09, "sine", 0.13);
  },
  heart2: (c) => {
    note(c, 85, 0, 0.1, "sine", 0.17);
  },
  heart3: (c) => {
    note(c, 80, 0, 0.05, "sine", 0.22);
    note(c, 80, 0.09, 0.11, "sine", 0.2);
  },
  ...
  buzz: (c) => {
    note(c, 185, 0, 0.11, "sawtooth", 0.13);
    note(c, 148, 0.13, 0.16, "sawtooth", 0.12);
  },
  // Pano hücresi açıldı / soru perdesi: kısa tok "çevir" sesi.
  flip: (c) => {
    note(c, 340, 0, 0.05, "square", 0.1);
    note(c, 180, 0.035, 0.11, "triangle", 0.11);
  },
  // Eliminasyon: alçalan ikili — can bittiğinde bir kez.
  elim: (c) => {
    note(c, 392, 0, 0.12, "sine", 0.13);
    note(c, 262, 0.11, 0.24, "sine", 0.14);
  },
  // Emote baloncuğu: çok kısa, çok hafif — akışı kirletmez.
  pop: (c) => {
    note(c, 1200, 0, 0.04, "sine", 0.07);
  },
};
  ...
  play(name: SfxName) {
    ...
    if (c)
      try {
        ...
        /* ses zorunlu değil, sessizce yut */
      }
  },
  isOn() {
    return enabled;
