...
for (const q of classics) {
  if (
    ...
    q.choices.length !== 4 ||
    !Array.isArray(q.choicesEn) ||
    q.choicesEn.length !== 4
  )
    fail(`Invalid classic question: ${q.id}`);
}
...
for (const p of circles) {
  if (
    ...
    fail(`Invalid circle prompt: ${p.answer}`);
}
if (new Set(circles.map((p) => `${p.category}|${p.answer}`)).size !== circles.length)
  fail("Duplicate incoming circle answers");
...
const skippedCircles = circles
  .filter((p) => existingCircleKeys.has(`${p.category}|${p.answer}`))
...
if (newCircles.length)
  fs.writeFileSync(
    circlePath,
    circleText.replace(closingPattern, `\n${insertion},\n];\n\nexport const normalizeCircleAnswer`),
  );
console.log(
  JSON.stringify(
    {
      classicFiles,
      circleFiles,
      classicAdded: newClassics.length,
      classicSkipped: skippedClassicIds,
      circleAdded: newCircles.length,
      circleSkipped: skippedCircles,
    },
    null,
    2,
  ),
